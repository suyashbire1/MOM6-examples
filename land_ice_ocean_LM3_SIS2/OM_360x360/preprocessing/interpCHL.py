...
C=midas.rectgrid.state(path=path_seawifs,grid=grid,fields=['chl'],verbose=True)
...
C2.var_dict['chl']['tunits']='seconds since 1900-01-01 00:00:00'
C2.rename_field('chl','chl_a')

path_out='seawifs.nc'
C2.write_nc(path_out,['chl_a'])
f=nc.Dataset(path_out,'a')
f.variables['time'].modulo = 'y'
f.sync()
f.close()
