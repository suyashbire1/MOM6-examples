...
S=Su.horiz_interp('ua',target=grid,method='bilinear')
S=Sv.horiz_interp('va',target=grid,method='bilinear',PrevState=S)
...
T=S.horiz_interp('umod',target=output_grid,method='bilinear')

T.fill_interior('umod')

T.var_dict['umod']['yax_data'] = output_grid.y_T[:,output_grid.im/4]

#T.umod = np.ma.filled(T.umod,0.0)
