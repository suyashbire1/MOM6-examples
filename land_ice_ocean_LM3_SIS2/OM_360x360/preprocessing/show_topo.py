...
#grid.Ish=nc.Dataset('edit_topog.nc').variables['ice_shelf_thickness'][:]
grid.wet[grid.D<=0.]=0.
...
def shade_stereo(var,grid,hem=1,fig=1,field=None,tlev=0,klev=0,vmin=None,vmax=None,cmap=plt.cm.spectral,scale=1.0,nlevs=0):
    ...
    if hem == -1:
	    m.drawparallels(np.arange(-90,-50,5))
    else:
	    m.drawparallels(np.arange(50,90,5))
    
south=grid.geo_region(y=(grid.latq[0],-40))
S=state(grid=grid,geo_region=south)
D=np.ma.masked_where(S.grid.D<=0.0,S.grid.D)
...
north=grid.geo_region(y=(40,grid.latq[-1]))
S=state(grid=grid,geo_region=north)
D=np.ma.masked_where(S.grid.D<=0.0,S.grid.D)
shade_stereo(D,S.grid,fig=2,hem=1,scale=1,vmin=0,vmax=6000,cmap=plt.cm.gist_stern)
S=state(grid=grid,geo_region=None)
show_regional_grid([grid.lonq[0],grid.lonq[-1]],[-60,60],fig=3,vmin=0,vmax=6000,cmap=plt.cm.gist_stern)
